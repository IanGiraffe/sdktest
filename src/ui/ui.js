// Helper function to display output
export function displayOutput(elementId, data) {
    const element = document.getElementById(elementId);
    element.textContent = JSON.stringify(data, null, 2);
    
    // Add copy button if it doesn't exist
    if (!element.parentNode.querySelector('.copy-btn')) {
        const copyBtn = document.createElement('button');
        copyBtn.className = 'copy-btn';
        copyBtn.style.cssText = 'padding: 8px; background: #28a745; border: none; border-radius: 4px; cursor: pointer; display: flex; align-items: center; justify-content: center;';
        
        // Create the copy icon
        const copyIcon = document.createElement('img');
        copyIcon.src = 'https://static.thenounproject.com/png/361082-200.png';
        copyIcon.style.cssText = 'width: 16px; height: 16px; filter: brightness(0) invert(1);';
        copyIcon.alt = 'Copy';
        copyBtn.appendChild(copyIcon);
        
        copyBtn.onclick = () => {
            navigator.clipboard.writeText(element.textContent).then(() => {
                copyIcon.style.filter = 'brightness(0) saturate(100%) invert(27%) sepia(51%) saturate(2878%) hue-rotate(346deg) brightness(104%) contrast(97%)'; // Green checkmark color
                copyIcon.src = 'data:image/svg+xml;base64,';
                setTimeout(() => {
                    copyIcon.src = 'https://static.thenounproject.com/png/361082-200.png';
                    copyIcon.style.filter = 'brightness(0) invert(1)';
                }, 2000);
            }).catch(err => {
                console.error('Failed to copy: ', err);
                copyIcon.style.filter = 'brightness(0) saturate(100%) invert(17%) sepia(94%) saturate(7151%) hue-rotate(359deg) brightness(95%) contrast(118%)'; // Red X color
                copyIcon.src = 'data:image/svg+xml;base64,';
                setTimeout(() => {
                    copyIcon.src = 'https://static.thenounproject.com/png/361082-200.png';
                    copyIcon.style.filter = 'brightness(0) invert(1)';
                }, 2000);
            });
        };
        element.parentNode.appendChild(copyBtn);
    }
}
